...
class DbMysqli extends Db{
	...
	static protected $cache=array();	// 数据缓存
    ...
    public function link() {
        if (is_null(self::$db_link)) {
            $link=new mysqli(C("DB_HOST"), C("DB_USER"), C("DB_PWD"), C("DB_NAME"), C("DB_PORT"));
            if (!$link) {
                error($link->connect_error); //数据库连接出错了请检查配置文件中的参数
            } else {
                self::$db_link = $link;
                self::setCharts();
            }
...
//     	exit($query);
    	...
    	if(self::$db_link->error){
    		throw new ExceptionStart(self::$db_link->error);
    	}
    	if ($this->lastquery) {
    		//自增id
