...
// {
//     if (preg_match("/^https?:\/\/\s{0}/i", $pathinfo)){
//         return $pathinfo;
//     }
//     //是否指定单入口
...
//         } elseif (strtoupper($info[0]) == 'StartPHP') {
//             $base = dirname(substr_replace($class, START_PATH, 0, 5));
//             $class = basename($class);
//         } elseif (in_array(strtoupper($info[0]), array("LIB", "ORG"))) {
//             $base = StartPHP_EXTEND_PATH;
//         } else {
//             //其它应用
...
function file_merge($files, $space = false, $tag = false)
{
    ...
    return $tag ? '<?php if(!defined("START_PATH")){exit("No direct script access allowed");}' . $str . "\t?>" : $str;
}
...
function compress($content)
{
    ...
    $end = false; //没结束如$v = "StartPHP"中的等号;
 ...
 * @throws
 */
function throw_exception($msg, $type = "StartPHPException", $code = 0)
{
...
//         include C('ERROR_TPL');
//     else
//         include StartPHP_TPL_PATH . 'halt.html';
//     exit;
// }
