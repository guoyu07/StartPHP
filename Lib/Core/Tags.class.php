...
final class Tags{
	...
	public function getArgs(){
		if(preg_match_all($this->pattern, $this->tplContent,$arr)){
			$args=array();
			foreach ($arr[1] as $v){
				$args[]=expToArr(filterArr(preg_split('/\s+/', $v)));
			}
			$con=array();
	...
	private function replace($sys){
		...
		$_SESSION['replace']=$str;
		if(!function_exists('replaceTag')){
			function replaceTag(){
				return $_SESSION['replace'];
			}
		}
		$this->tplContent=preg_replace_callback($this->pattern,'replaceTag', $this->tplContent);
